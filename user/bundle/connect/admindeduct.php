...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    if (mysqli_stmt_execute($updateStmt)) {
        // Log the transaction in withdrawal_request table
        $with_type = strtoupper($crypto) . ' ' . number_format($amount, 8);
        $status = 'Approved';
        $date = date('Y-m-d H:i:s');
        $wal_address = 'Admin Deduction';

        $logSql = "INSERT INTO withdrawal_request (email, amount, with_type, wal_address, acc_name, bank_name, acc_number, rou_number, status, date_created) VALUES (?,?,?,?,?,?,?,?,?,?)";
        $logStmt = mysqli_prepare($dbconnected, $logSql);
        $usdAmount = 0; // You can calculate USD value if needed
        $emptyStr = '';
        mysqli_stmt_bind_param($logStmt, "sdssssssss", $email, $usdAmount, $with_type, $wal_address, $emptyStr, $emptyStr, $emptyStr, $emptyStr, $status, $date);
        mysqli_stmt_execute($logStmt);
        mysqli_stmt_close($logStmt);

        echo json_encode([
            'success' => true,
            'message' => 'Successfully deducted ' . $amount . ' ' . strtoupper($crypto),
